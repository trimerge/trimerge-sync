...
describe('TrimergeClient: 2 users', () => {
  ...
  describe('restore commit', () => {
    it('restores a commit', async () => {
      const store = newStore();
      const clientA = makeClient('a', store, {
        computeRef: getRefListComputeRef([
          '1', // original
          '2', // edit 1
          '3', // edit 2
          '4', // restore
      ...
      await clientA.updateDoc(originalDoc, { message: 'message' });
      await clientA.updateDoc({ foo: 'baz' }, { message: 'message' });
      await clientA.updateDoc(
        { foo: 'baz', baz: 'qux' },
        { message: 'message' },
      );
      await clientA.restoreCommit('1', { message: 'restore' });

      expect(clientA.doc).toBe(originalDoc);
    });

    it('collaborating user restore', async () => {
      const store = newStore();
      const clientA = makeClient('a', store, {
        computeRef: getRefListComputeRef([
          '1', // original
      ...
      expect(clientA.doc).toEqual({ foo: 'baz', baz: 'qux' });
      expect(clientB.doc).toEqual({ foo: 'baz', baz: 'qux' });

      await Promise.all([
        clientA.restoreCommit('1', { message: 'restore' }),
        clientB.updateDoc({ foo: 'baz', baz: 'baz' }, { message: 'edit' }),
      ]);

      await timeout();

      expect(clientA.doc).toMatchInlineSnapshot(`
        {
          "baz": "baz",
          "foo": "bar",
        }
      `);
      expect(clientB.doc).toMatchInlineSnapshot(`
        {
          "baz": "baz",
          "foo": "bar",
        }
    ...
    it('attempting to restore last non temp head or a temp commit is a no-op', async () => {
      const store = newStore();
      const clientA = makeClient('a', store, {
        computeRef: getRefListComputeRef([
          '1', // original
      ...
      const doc = clientA.doc;
      // attempt to restore a temp commit
      await clientA.restoreCommit('merge', { message: 'restore' });

      await timeout();

      expect(clientA.doc).toBe(doc);

      // attempt to restore the last non-temp commit
      await clientA.restoreCommit('4', { message: 'restore' });
