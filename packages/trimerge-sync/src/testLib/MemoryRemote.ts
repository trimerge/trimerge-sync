  ...
  implements Remote<CommitMetadata, Delta, Presence>
{
  ...
  send(event: SyncEvent<CommitMetadata, Delta, Presence>): void {
    ...
    this.logger?.event?.({
      ...
      sourceId: 'remote',
      payload: {
        senderId: `COORDINATING_LOCAL_STORE:${this.clientInfo.clientId}`,
        event,
      },
    });
  ...
  async sendInitialEvents(lastSyncCursor: string | undefined): Promise<void> {
...
export class MemoryServer<CommitMetadata, Delta, Presence> {
  ...
  constructor(readonly store: MemoryStore<CommitMetadata, Delta, Presence>) {}
  ...
  remote(
    ...
    if (this.remoteMap.has(clientKey)) {
      throw new Error(
        `A remote with userId ${clientInfo.userId} and clientId ${clientInfo.clientId} has already been created.`,
      );
    }
  ...
  emit(
    ...
    recipientClientInfo: ClientInfo,
  ): void {
    const remote = this.remoteMap.get(this.getClientKey(recipientClientInfo));
    invariant(
      remote,
      `No remote found for userId ${recipientClientInfo.userId} and clientId ${recipientClientInfo.clientId}`,
    );
    remote.emit(event);
  }
  ...
  protected async broadcast(
    ...
    for (const [clientKey, remote] of this.remoteMap.entries()) {
      ...
      if (
        senderClientInfo &&
        this.getClientKey(senderClientInfo) === clientKey
      ) {
        continue;
      }
      remote.emit(event);
