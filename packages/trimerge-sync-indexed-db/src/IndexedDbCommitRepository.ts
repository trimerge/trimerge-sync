  ...
  implements CommitRepository<CommitMetadata, Delta, Presence>
{
  ...
  async acknowledgeRemoteCommits(
    ...
    const tx = (await this.dbInfo).db.transaction(['commits'], 'readwrite');
    const commits = tx.objectStore('commits');
  ...
  private async connect(reconnect: boolean = false): Promise<{
    db: IDBPDatabase<TrimergeSyncDbSchema<CommitMetadata, Delta>>;
    localStoreId: string;
  }> {
    if (reconnect) {
  ...
  async addCommits(
    ...
    const { db, localStoreId } = await this.dbInfo;
