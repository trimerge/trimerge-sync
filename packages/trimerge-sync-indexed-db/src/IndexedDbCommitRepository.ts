  ...
  async addCommits(
    ...
    for (const commit of commits) {
      ...
      promises.push(
        (async () => {
          try {
            // FIXME: there is a weird order of operations issue here.
            // if we attempt to get the localStoreId before checking if the
            // commit exists, the call to get the commit by ref will fail.
            // If we do this get first though, the transaction remains valid.
            await commitsDb.get(commit.ref);

            // We only attach store metadata to commits generated locally.
            // we attach store metadata even if the commit already exists in the db
            // so that we record that this commit was generated by the local client.
